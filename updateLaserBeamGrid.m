...
function [p,l] = updateLaserBeamGrid(angle, range, Tl, R, C, Xmax, Ymax)
global occ_odds;
global occ_grid;

P_Occ = 0.6;
Odds_Occ = P_Occ / (1 - P_Occ);
Odds_nOcc = 1 / Odds_Occ;
...
% map world points to integer coordinates
...
occ_odds(I2, J2) = occ_odds(I2, J2) * Odds_Occ;
occ_grid(I2, J2) = occ_odds(I2, J2) / (occ_odds(I2, J2) + 1);
...
for k=1:length(l)-1 %skip the target pixel
    try
    occ_odds(l(k,1),l(k,2)) = occ_odds(l(k,1),l(k,2)) * Odds_nOcc; % free pixels
    occ_grid(l(k,1),l(k,2)) = occ_odds(l(k,1),l(k,2)) / (occ_odds(l(k,1),l(k,2)) + 1);
    catch
        keyboard
    end
end
